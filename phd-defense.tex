...
\begin{frame}[t]{(C3) Alternative Feature Selection -- Approach}
	...
	\begin{itemize}
		...
		%JB: for fixed feature-set sizes, resulting constraint becomes linear
	\end{itemize}
\end{frame}
...
\begin{frame}[t]{(C4) Constrained Subgroup Discovery -- Empirical Results}
	...
	\begin{figure}
		...
		\begin{subfigure}[t]{0.32\textwidth}
			...
			%JB: six SD methods and sic feature-cardinality thresholds
			%JB: on average, heuristics "Beam" and "BI" beat solver-based search due to timeouts (here, largest timeout setting of 2048~s); however, even without latter, only small difference
