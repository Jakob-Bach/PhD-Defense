...
\newcommand{\longcite}[1]{\citeauthor{#1} (\citeyear{#1}) \cite{#1}}  % we generally have numeric style but partly use "Author (year) [ref]" as well
...
\begin{frame}[t]{(C4) Constrained Subgroup Discovery -- Empirical Results}
	...
	\begin{figure}
		...
		\begin{subfigure}[t]{0.32\textwidth}
			\centering
			\includegraphics[width=\textwidth, trim={15 -29 15 15}, clip]{plots/csd-timeouts-nwracc.pdf} % actual bottom trimming should be different, but we use a negative number to manually top-align the figures (instead of only the captions)
...
\begin{frame}[t]{Conclusions}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item (C4) Using constraints for feature selection in subgroup discovery: Bach (2025) \cite{bach2025subgroup}, \longcite{bach2024using} % first reference in this line doesn't have year (is included in "howpublished"), so we manually cite it
		\end{itemize}
